...
public abstract class Service : IService
{
    private class ServiceContext(
        ...
        Func<Task> getTask,
        Func<CancellationTokenSource> getCancellationTokenSource
    )
    {
    ...
    protected Service(string? name, Service downstreamLogger)
        : this(name, downstreamLogger.Logger) { }
    ...
    public async Task Start(CancellationToken startupCancellationToken = default)
    {
        ...
        _ = Factory!.StartNew(
            async () =>
            {
                Logger.Log(
                    LogLevel.Debug,
                    $"Running on thread: \"{Thread.CurrentThread.Name}\" (#{Environment.CurrentManagedThreadId})."
                );
                TaskCompletionSource serviceCompletionSource = new();
                using CancellationTokenSource serviceCancellationTokenSource = new();

                ServiceContext context = Context = new(
                    () => state,
                    () => serviceCompletionSource.Task,
                ...
                try
                {
                    ...
                    catch (Exception onRunException)
                    {
                        if (
                            (
                                onRunException
                                is not OperationCanceledException onStopCancelledException
                            )
                            || (
                                serviceCancellationTokenSource.Token
                                != onStopCancelledException.CancellationToken
                            )
                        )
                        {
        ...
        void logException(Exception exception) =>
            Logger.Log(
                LogLevel.Fatal,
                $"Fatal error when {state}: [{exception.GetType().Name}] {exception.Message}{(exception.StackTrace != null ? $"\n{exception.StackTrace}" : "")}"
            );
    ...
    protected static async Task WatchDog(Service[] services, CancellationToken cancellationToken)
    {
        ...
        catch (Exception exception)
        {
            if (
                ...
                && cancellationToken == operationCanceledException.CancellationToken
            )
            {
                return;
            }
