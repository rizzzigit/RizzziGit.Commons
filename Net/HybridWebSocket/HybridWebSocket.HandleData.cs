...
public partial class HybridWebSocket
{
    ...
    private async Task HandleData(CompositeBuffer data, CancellationToken cancellationToken)
    {
        ...
        switch (data[0])
        {
            ...
            case DATA_REQUEST:
                await HandleRequest(
                    data.Slice(1, 5).ToUInt32(),
                    new(data.Slice(5, 9).ToUInt32(), data.Slice(9)),
                    cancellationToken
                );
                break;
            ...
            case DATA_ERROR_RESPONSE:
            {
                ...
                HandleErrorResponse(
                    data.Slice(1, 5).ToUInt32(),
                    data.Slice(13, 13 + nameLength).ToString(),
                    data.Slice(13 + nameLength).ToString()
                );
                break;
            }
            ...
            default:
                throw new InvalidDataException(
                    $"Unknown data type: 0x{Convert.ToHexString(new byte[] { data[0] })}"
                );
        }
    }
}
